...
# -s: 入力されたパスワードを表示させない
read -s -p "パスワード名を入力してください：" password
password="${password:?パスワードを設定してください。}"

# 複合化したデータを一時ファイルに保存
# 標準エラー出力(2)を/dev/nullにすてる。つまり、エラーメッセージを表示しない。
gpg -d password.org > password.txt 2> /dev/null
...
read -p "サービス名を入力してください：" serviceName
# 複合化したデータを一時ファイルに保存
gpg -d password.pgp > password.txt 2> /dev/null
# serviceNameに対するpassをpasswords.txtファイルから取得
